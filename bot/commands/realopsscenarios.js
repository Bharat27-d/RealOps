...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      // Send the embeds to the channel first
      await interaction.channel.send({ content: `<@${user.id}>`, embeds });
      
      // Then confirm with ephemeral reply
      ...
      if (!interaction.replied && !interaction.deferred) {
        try {
          await interaction.reply({ content: 'An error occurred while sending the scenarios.', flags: 64 });
        } catch (err) {}
