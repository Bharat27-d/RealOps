const HomePage={async render(){const[t,e,a,r]=await Promise.all([API.getStats(),API.getEvents(),API.getStaff(),API.getPartnerships()]),i=(e||[]).slice(0,3),s=(a||[]).filter(o=>o.status==="active").slice(0,6);return`
            ...
            <div class="grid grid-3 reveal">
              ${i.map((o,n)=>`
                <div class="bento-card ambient-shadow reveal reveal-delay-${n+1}" style="padding: 24px;">
                  ${o.image?`<img class="event-card-banner" src="${App.escapeHtml(o.image)}" alt="${App.escapeHtml(o.title)}" loading="lazy" style="border-radius: 8px; margin-bottom: 16px;">`:'<div class="event-card-banner-placeholder" style="border-radius: 8px; margin-bottom: 16px;">\u{1F69B}</div>'}
                  <div class="event-card-body" style="padding: 0;">
                    <h3 class="event-card-title" style="font-size: 20px; font-weight: 500; margin-bottom: 12px;">${App.escapeHtml(o.title)}</h3>
                    <div class="event-card-meta" style="margin-bottom: 16px;">
                      ...
                      ${o.time?`
                        <div class="event-card-meta-item" style="color: var(--color-text-secondary); font-family: var(--font-mono); font-size: 13px;">
                          ...
                          ${App.escapeHtml(o.time)} UTC
                        </div>
                      `:""}
                      ${o.server?`
                        <div class="event-card-meta-item" style="color: var(--color-text-secondary); font-family: var(--font-mono); font-size: 13px;">
                          <span class="material-symbols-outlined" style="font-size: 16px;">dns</span>
                          ${App.escapeHtml(o.server)}
                        </div>
                      `:""}
                    </div>
                    <div class="event-card-footer">
                      <span class="badge badge-success" style="background: rgba(34,197,94,0.1); color: #22C55E; font-family: var(--font-mono);">${App.escapeHtml(o.status||"Scheduled")}</span>
                      ${o.attendance?`<span class="event-card-meta-item" style="font-family: var(--font-mono); color: var(--color-text-secondary);">\u{1F464} ${o.attendance}</span>`:""}
                    </div>
                  </div>
                </div>
              `).join("")}
            ...
            <div class="grid grid-3 reveal">
              ${s.map((o,n)=>`
                <div class="bento-card ambient-shadow reveal reveal-delay-${n+1}" style="padding: 24px; display: flex; flex-direction: column; align-items: center; text-align: center;">
                  ${o.avatar?`<img class="staff-avatar" src="${App.escapeHtml(o.avatar)}" alt="${App.escapeHtml(o.name)}" loading="lazy" style="width: 80px; height: 80px; border-radius: 50%; border: 2px solid var(--color-border); margin-bottom: 16px;">`:`<div class="staff-avatar-placeholder" style="width: 80px; height: 80px; border-radius: 50%; border: 2px solid var(--color-border); margin-bottom: 16px; display: flex; align-items: center; justify-content: center; font-size: 24px;">${(o.name||"?").charAt(0).toUpperCase()}</div>`}
                  <h3 class="staff-name" style="font-size: 20px; font-weight: 500; color: var(--color-text); margin-bottom: 4px;">${App.escapeHtml(o.name)}</h3>
                  <p class="staff-position" style="font-size: 14px; color: var(--color-text-secondary); margin-bottom: 12px;">${App.escapeHtml(o.position||"Team Member")}</p>
                  <span class="staff-department" style="font-family: var(--font-mono); font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--color-primary); background: rgba(255, 107, 53, 0.1); padding: 4px 8px; border-radius: 4px;">${App.escapeHtml(o.department)}</span>
                </div>
              `).join("")}
            </div>
            <div class="text-center reveal" style="margin-top: var(--space-8);">
        ...
        <section class="section" style="width: 100%; margin-bottom: 40px;">
          <div class="container" style="max-width: 100%;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 24px;">
              ${e.map(r=>`
                <a href="${r.url||"#"}" target="_blank" rel="noopener" class="bento-card ambient-shadow reveal" style="padding: 32px 24px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-decoration: none; text-align: center; transition: transform 0.3s ease, border-color 0.3s ease;">
                  ${r.logo?`
                    <div style="height: 80px; display: flex; align-items: center; justify-content: center; margin-bottom: 16px;">
                      <img src="${r.logo}" alt="${r.name}" loading="lazy" style="max-width: 140px; max-height: 80px; object-fit: contain;">
                    </div>
                  `:`
                  ...
                  `}
                  <h3 style="font-size: 18px; font-weight: 600; color: var(--color-text); margin-bottom: 8px;">${r.name||"Partner"}</h3>
                  ${r.description?`<p style="font-size: 14px; color: var(--color-text-secondary); line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">${r.description}</p>`:""}
                </a>
              `).join("")}
            </div>
            `:`
      ...
      <div class="bento-card ambient-shadow reveal reveal-delay-${l%6+1}" data-department="${App.escapeHtml(s.department||"General")}" style="padding: 24px; display: flex; flex-direction: column; align-items: center; text-align: center;">
        ...
        ${s.roles&&s.roles.length>0?`
          <div class="staff-roles" style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center; margin-bottom: 16px;">
            ${s.roles.slice(0,3).map(o=>`
              <span class="staff-role-badge" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 2px 8px; border-radius: 12px; font-size: 11px; color: var(--color-text-secondary);">${App.escapeHtml(typeof o=="string"?o:o.name||o)}</span>
            `).join("")}
                  ...
                  <div class="bento-card ambient-shadow reveal reveal-delay-${a%4+1}" style="padding: 32px; display: flex; flex-direction: column;">
                    ...
                    ${e.roles&&e.roles.length>0?`
                      <div class="recruitment-roles" style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 24px;">
                        ${e.roles.map(r=>`
                          <span class="recruitment-role-tag" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 4px 12px; border-radius: 9999px; font-size: 12px; color: var(--color-text-secondary); font-family: var(--font-mono);">${App.escapeHtml(r.name||r)}</span>
                        `).join("")}
                      </div>
                    `:""}
...
`).filter(r=>r.trim()).map(r=>`
                            <li style="font-size: 14px; color: var(--color-text-secondary); display: flex; align-items: flex-start; gap: 8px;">
                              ...
                              <span>${App.escapeHtml(r.trim().replace(/^[-•*]\s*/,""))}</span>
                            </li>
                          `).join("")}
                        </ul>
                      </div>
    ...
    `}},StatsPage={async render(){const[t,e,a,r]=await Promise.all([API.getStats(),API.getStaff(),API.getPartnerships(),API.getEvents()]),i=new Date().getFullYear()-(t?.foundedYear||2021),s=(e||[]).filter(o=>o.status==="active").length,l=[...new Set((e||[]).map(o=>o.department).filter(Boolean))];return`
      <main class="flex-grow relative z-10 pt-[120px] pb-xl px-4 md:px-lg mx-auto w-full flex flex-col gap-xl" style="padding-top: 120px; padding-bottom: 64px; max-width: var(--max-width); margin: 0 auto;">
          ...
          <div class="container" style="max-width: 100%;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 12px; margin-bottom: 24px;">
              ...
              <div class="bento-card ambient-shadow reveal reveal-delay-3" style="padding: 32px; display: flex; flex-direction: column; justify-content: space-between; min-height: 200px;">
                ...
                <div>
                  <div style="font-size: 48px; font-weight: 600; color: var(--color-text); letter-spacing: -0.04em; line-height: 1.1;" data-count="${(r||[]).length}">0</div>
    ...
    `},async handleSubmit(t){t.preventDefault();const e=document.getElementById("contact-form"),a=document.getElementById("contact-submit"),r=new FormData(e),i={name:r.get("name"),email:r.get("email"),discord:r.get("discord"),subject:r.get("subject"),message:r.get("message")};a.disabled=!0,a.innerHTML=`
