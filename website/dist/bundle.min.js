const HomePage={async render(){const[t,e,a,o]=await Promise.all([API.getStats(),API.getEvents(),API.getStaff(),API.getPartnerships()]),i=(e||[]).slice(0,3),r=(a||[]).filter(s=>s.status==="active").slice(0,6);return`
        ...
        <div class="container">
          ...
          ${i.length>0?`
            <div class="grid grid-3 reveal">
              ${i.map((s,n)=>`
                <div class="bento-card ambient-shadow reveal reveal-delay-${n+1}" style="padding: 24px;">
                  ${s.image?`<img class="event-card-banner" src="${App.escapeHtml(s.image)}" alt="${App.escapeHtml(s.title)}" loading="lazy" style="border-radius: 8px; margin-bottom: 16px;">`:'<div class="event-card-banner-placeholder" style="border-radius: 8px; margin-bottom: 16px;">\u{1F69B}</div>'}
                  <div class="event-card-body" style="padding: 0;">
                    <h3 class="event-card-title" style="font-size: 20px; font-weight: 500; margin-bottom: 12px;">${App.escapeHtml(s.title)}</h3>
                    ...
                    <div class="event-card-footer">
                      <span class="badge badge-success" style="background: rgba(34,197,94,0.1); color: #22C55E; font-family: var(--font-mono);">${App.escapeHtml(s.status||"Scheduled")}</span>
                      ${s.attendance?`<span class="event-card-meta-item" style="font-family: var(--font-mono); color: var(--color-text-secondary);">\u{1F464} ${s.attendance}</span>`:""}
                    </div>
                  </div>
                </div>
              `).join("")}
            ...
            <div class="grid grid-3 reveal">
              ${r.map((s,n)=>`
                <div class="bento-card ambient-shadow reveal reveal-delay-${n+1}" style="padding: 24px; display: flex; flex-direction: column; align-items: center; text-align: center;">
                  ...
                  <span class="staff-department" style="font-family: var(--font-mono); font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--color-primary); background: rgba(255, 107, 53, 0.1); padding: 4px 8px; border-radius: 4px;">${App.escapeHtml(s.department)}</span>
                </div>
              `).join("")}
            </div>
            <div class="text-center reveal" style="margin-top: var(--space-8);">
    ...
    `},getStatusBadgeClass(t){switch((t||"").toLowerCase()){case"scheduled":return"badge-info";case"sent":case"announced":return"badge-success";case"completed":return"badge-primary";case"cancelled":return"badge-warning";default:return"badge-info"}},initFilters(){const t=document.querySelectorAll("#event-filters .filter-btn"),e=document.querySelectorAll("#events-grid .event-card");t.forEach(a=>{a.addEventListener("click",()=>{t.forEach(i=>i.classList.remove("active")),a.classList.add("active");const o=a.dataset.filter;e.forEach(i=>{o==="all"||i.dataset.status===o?i.style.display="":i.style.display="none"})})})}},TeamPage={async render(){const t=await API.getStaff(),e=["All",...new Set((t||[]).flatMap(r=>r.departments||[r.department]).filter(Boolean))],a=[],o=[];(t||[]).forEach(r=>{(r.position||"").toLowerCase()==="project manager"?a.push(r):o.push(r)});const i=(r,l)=>{const s=r.departments||[r.department||"General"];return`
      <div class="bento-card ambient-shadow reveal reveal-delay-${l%6+1}" data-departments="${App.escapeHtml(s.join(","))}" style="padding: 24px; display: flex; flex-direction: column; align-items: center; text-align: center;">
        ${r.avatar?`<img class="staff-avatar" src="${App.escapeHtml(r.avatar)}" alt="${App.escapeHtml(r.name)}" loading="lazy" style="width: 80px; height: 80px; border-radius: 50%; border: 2px solid var(--color-border); margin-bottom: 16px;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
             <div class="staff-avatar-placeholder" style="display:none; width: 80px; height: 80px; border-radius: 50%; border: 2px solid var(--color-border); margin-bottom: 16px; align-items: center; justify-content: center; font-size: 24px;">${(r.name||"?").charAt(0).toUpperCase()}</div>`:`<div class="staff-avatar-placeholder" style="width: 80px; height: 80px; border-radius: 50%; border: 2px solid var(--color-border); margin-bottom: 16px; display: flex; align-items: center; justify-content: center; font-size: 24px;">${(r.name||"?").charAt(0).toUpperCase()}</div>`}
        ...
        <span class="staff-department" style="font-family: var(--font-mono); font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--color-primary); background: rgba(255, 107, 53, 0.1); padding: 4px 8px; border-radius: 4px; margin-bottom: 16px;">${App.escapeHtml(s.join(", "))}</span>
        ...
        ${r.roles&&r.roles.length>0?`
          <div class="staff-roles" style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center; margin-bottom: 16px;">
            ${r.roles.slice(0,3).map(n=>`
              <span class="staff-role-badge" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 2px 8px; border-radius: 12px; font-size: 11px; color: var(--color-text-secondary);">${App.escapeHtml(typeof n=="string"?n:n.name||n)}</span>
            `).join("")}
            ${r.roles.length>3?`<span class="staff-role-badge" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 2px 8px; border-radius: 12px; font-size: 11px; color: var(--color-text-secondary);">+${r.roles.length-3}</span>`:""}
          </div>
        `:""}
            ...
            ${t&&t.length>0?`
              ${e.length>2?`
                <div class="filter-bar reveal" id="team-filters" style="display: flex; justify-content: center; flex-wrap: wrap; gap: 8px; margin-bottom: 40px;">
                  ${e.map(r=>`
                    <button class="filter-btn glass-button-secondary ${r==="All"?"active":""}" data-filter="${r==="All"?"all":r}" style="padding: 8px 16px; font-size: 14px;">${App.escapeHtml(r)}</button>
                  `).join("")}
                </div>
              `:""}
    ...
    `},formatStatus(t){switch((t||"").toLowerCase()){case"active":return"Active";case"inactive":return"Inactive";case"leave":return"On Leave";default:return"Active"}},initFilters(){const t=document.querySelectorAll("#team-filters .filter-btn"),e=document.querySelectorAll(".bento-card[data-departments]");t.forEach(a=>{a.addEventListener("click",()=>{t.forEach(i=>{i.classList.remove("active"),i.style.background="transparent",i.style.color="var(--color-text)"}),a.classList.add("active"),a.style.background="rgba(255, 255, 255, 0.05)",a.style.color="var(--color-primary)";const o=a.dataset.filter;e.forEach(i=>{const r=(i.dataset.departments||"").split(",");o==="all"||r.includes(o)?i.style.display="":i.style.display="none"})})})}},RecruitmentPage={async render(){const t=await API.getRecruitment();return`
      <main class="flex-grow relative z-10 pt-[120px] pb-xl px-4 md:px-lg mx-auto w-full flex flex-col gap-xl" style="padding-top: 120px; padding-bottom: 64px; max-width: var(--max-width); margin: 0 auto;">
...
`).filter(o=>o.trim()).map(o=>`
    ...
    `}},StatsPage={async render(){const[t,e,a,o]=await Promise.all([API.getStats(),API.getStaff(),API.getPartnerships(),API.getEvents()]),i=new Date().getFullYear()-(t?.foundedYear||2021),r=(e||[]).filter(s=>s.status==="active").length,l=[...new Set((e||[]).map(s=>s.department).filter(Boolean))];return`
      <main class="flex-grow relative z-10 pt-[120px] pb-xl px-4 md:px-lg mx-auto w-full flex flex-col gap-xl" style="padding-top: 120px; padding-bottom: 64px; max-width: var(--max-width); margin: 0 auto;">
          ...
          <div class="container" style="max-width: 100%;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 12px;">
              ...
              <!--<div class="bento-card ambient-shadow reveal reveal-delay-2" style="padding: 24px; display: flex; align-items: center; gap: 16px;">
                ...
                <div>
                  <div style="font-size: 24px; font-weight: 600; color: var(--color-text); line-height: 1;" data-count="${r}">0</div>
              ...
              <div class="bento-card ambient-shadow reveal reveal-delay-3" style="padding: 24px; display: flex; align-items: center; gap: 16px;">
                ...
                <div>
                  <div style="font-size: 24px; font-weight: 600; color: var(--color-text); line-height: 1;" data-count="${l.length}">0</div>
